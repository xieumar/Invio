...
import { getAllInvoices, saveInvoice, deleteInvoice as dbDelete, seedInvoices } from "@/lib/db";
...
interface InvoiceContextValue {
  invoices: Invoice[];
  loading: boolean;
  createInvoice: (data: InvoiceFormData, status: InvoiceStatus) => Promise<Invoice>;
...
export function InvoiceProvider({ children }: { children: React.ReactNode }) {
  ...
  const createInvoice = useCallback(async (data: InvoiceFormData, status: InvoiceStatus): Promise<Invoice> => {
  ...
  const updateInvoice = useCallback(async (id: string, data: InvoiceFormData) => {
  ...
  return (
    <InvoiceContext.Provider value={{ invoices, loading, createInvoice, updateInvoice, deleteInvoice, markAsPaid, getInvoice }}>
      {children}
